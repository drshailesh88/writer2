...
- [x] T031 [US4] Create Razorpay subscription API route `app/api/razorpay/create-subscription/route.ts` — POST handler: require auth, accept `{ planType }`, look up plan ID from env vars, call `razorpay.subscriptions.create()`, return `{ subscriptionId, razorpayKeyId }`. Passes `clerkUserId` as Razorpay note for webhook mapping.
- [x] T032 [US4] Create Razorpay webhook handler `app/api/razorpay/webhook/route.ts` — POST handler: verify webhook signature using HMAC SHA256 (raw body + RAZORPAY_WEBHOOK_SECRET), handle events: `subscription.activated` (call Convex `subscriptions.activateFromWebhook` + update user tier), `subscription.charged` (update billing period), `subscription.cancelled` (call Convex `subscriptions.updateFromWebhook` to "cancelled"), `payment.failed` (set subscription to "paused" for 3-day grace). Always respond 200. Idempotent via `razorpaySubscriptionId`.
- [x] T033 [US4] Design the pricing page using **Frontend Design Skill + Opus model** — 3-column feature comparison table (Free / Basic INR 1,000/mo / Pro INR 2,000/mo), check/cross marks for features, "Subscribe to Basic" primary CTA, "Coming Soon" for Pro, sign-up prompt for anonymous visitors. Professional, clean layout. Mobile: stack columns vertically.
...
- [x] T038 [US5] Create cancel subscription API route `app/api/razorpay/cancel-subscription/route.ts` — POST handler: require auth, get user's active subscription from Convex, call `razorpay.subscriptions.cancel(subscriptionId, { cancel_at_cycle_end: true })`, return `{ status, effectiveDate }`.
- [x] T039 [US5] Design the subscription management page using **Frontend Design Skill + Opus model** — card layout: current plan badge, renewal date, usage progress bars (plagiarism X/Y, AI detection X/Y), "Cancel Subscription" danger button with confirmation dialog, grace period warning banner (if payment failed). Clean, trustworthy design.
- [x] T040 [US5] Implement subscription management page `app/(protected)/account/subscription/page.tsx` — query Convex `subscriptions.getByUser` and `users.getUsage` for data. Show: plan name badge, renewal date (formatted), usage progress bars using shadcn/ui Progress, "Cancel Subscription" button → shadcn/ui AlertDialog confirmation ("Your subscription will remain active until [date]"). Grace period warning using shadcn/ui Alert (variant: destructive) if subscription status is "paused".
- [x] T041 [US5] Add "Subscription" and "Pricing" links to navigation in `components/nav-shell.tsx` — added to both desktop user dropdown menu and mobile nav sheet. Uses CreditCard and Tag icons.
...
- [x] T042 [US6] Verify and test the cron job created in T007 (`convex/crons.ts`) — `resetAllUsageCounters` internal mutation in `convex/users.ts` resets `plagiarismChecksUsed`, `aiDetectionChecksUsed`, and `deepResearchUsed` to 0 for ALL users. Cron calls it on the 1st of each month at 00:00 UTC.
